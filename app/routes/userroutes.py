...
def update_user_route(user_id:int, data:UserUpadte , db:Session = Depends(get_db), current_user:User= Depends(get_current_user)):
...
def delete_user_route(user_id:int , db:Session = Depends(get_db), current_user:User= Depends(require_roles(["admin"]))):
...
def patch_user_role(user_id:int, data:UpdateRoleSchema, db:Session=Depends(get_db), current_user:User= Depends(require_roles(["admin"]))):
